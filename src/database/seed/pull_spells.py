...
""" Command to pull spells.json script
docker exec -it dnd_discord_bot python -m database.seed.pull_spells
"""

# Target Repository Parameters
REPO_OWNER = "5etools-mirror-3"
REPO_NAME = "5etools-src"
DIRECTORY_PATH = "data/spells"
BRANCH = "main"

LOCAL_OUTPUT_DIR = Path(__file__).resolve().parent / "spells"

async def fetch_spells_directory_manifest(client: httpx.AsyncClient) -> list:
...
async def download_raw_file(client: httpx.AsyncClient, file_metadata: dict) -> None:
    ...
    # Optimization: Filter out directory indexes, legends, or non-spell files
    if not file_name.startswith("spells-") or not file_name.endswith(".json"):
        return
...
async def sync_entire_spells() -> None:
    """Orchestrates directory discovery and schedules concurrent downloads."""
    LOCAL_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    async with httpx.AsyncClient() as client:
        try:
            directory_contents = await fetch_spells_directory_manifest(client)
            ...
            if not tasks:
                print("⚠️ No matching spell files discovered in the target repository layout.")
                return
                
            print(f"⚡ Batch pipeline ready. Syncing {len(tasks)} targeted spell targets...")
...
if __name__ == "__main__":
    asyncio.run(sync_entire_spells())